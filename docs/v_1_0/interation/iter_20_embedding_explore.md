# 1. 目标
检查现有 embedding 模型是否合适，不合适则更换，并需要支持更多模型。
...
reranker（精排，非 embedding）：默认 `BAAI/bge-reranker-base`（中英双语），轻量备选 `cross-encoder/ms-marco-MiniLM-L-6-v2`。
...
- **多模型激活**：`RAG_ACTIVE_EMBEDDINGS`（默认 `en,zh`）决定检索时同时查哪几个 collection；默认别名由 `EMBEDDING_MODEL` 环境变量控制（默认 `en`）。
...
1. **默认是英文模型**：`en`（MiniLM）是默认 + 默认激活含 `en,zh`。若主语料是中文，MiniLM 会拖低召回质量。
2. **只支持本地 HF 模型**：无法接云端 API embedding（如需更高质量 / 不想占本地资源时没有选项）。
...
## 4.3. 云端 API embedding（先讨论，倾向暂不做）
