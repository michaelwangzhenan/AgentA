...
**是什么**：学而时习是 AgentA 的学习模块，让你在 AI 协助下按"定计划 → 做测验 → 间隔复习"三步把知识学透。

**AI 能力从哪来**：三项功能背后都是同一套能力组合——

- **模型**：聊天框里选的那个大模型（提供生成、批改等能力）
- **KB**：入库文档，出题 / 答疑时基于它检索（RAG）
- **skills**：三个业务 skill 指导模型怎么用工具——`study-planner`（拟计划）、`quiz-maker`（出题）、`srs-review`（复习）

**页面长什么样**：

- 顶部三个 Tab：**计划 / 测验 / 复习**，对应三项功能。
- 右侧一个可收起的 **AI 助手**面板：需要 AI 现场生成的操作（拟计划、出题）在这里发起，AI 干完活后左侧对应 Tab 自动刷新，不用切到聊天页。
- 标题栏 **帮助** 按钮：随时调出 / 收起使用说明。

**两种操作方式**（关键）：能本地算清楚的直接在页面做；要 AI 生成新内容的才去 AI 助手说一句。

| 子模块 | 页面内直接做 | 需要去 AI 助手说 |
|---|---|---|
| 计划 | 手动建计划、改任务状态、设为当前、放弃 | 让 AI 按目标自动拆解阶段任务 |
| 测验 | 作答、提交批改、归档 | 出题 |
| 复习 | 手动建卡片、自评打分、暂停 / 恢复 / 归档 | （无） |

> 说明："用没用 AI"和"要不要去 AI 助手"是两回事。比如简答批改底层调了大模型，但已封装进"提交批改"按钮里，页面点一下即可；而出题没有页面入口，必须去 AI 助手发起。

### 2.8.2 计划操作指南

**是什么 / 有什么用**：把一个学习目标拆成若干阶段任务，逐条跟踪进度，知道学到哪了。

| 想做什么 | 在哪做 | 怎么做 |
|---|---|---|
| 新建一个计划 | 计划 Tab | 点"新建计划"，填目标和任务 |
| 让 AI 帮我拟计划 | 右侧 AI 助手 | 说"帮我做一份 8 周的 ML 学习计划"，完成后列表自动出现 |
| 更新任务进度 | 计划 Tab | 点任务前的状态图标循环切换：待办 → 已完成 → 已跳过 |
| 切换当前主攻计划 | 计划卡片 | 点"设为当前" |
| 放弃一个计划 | 计划卡片 | 点"放弃" |

### 2.8.3 测验操作指南

**是什么 / 有什么用**：基于 KB 里的文档出题，检验你对资料的掌握程度。

| 想做什么 | 在哪做 | 怎么做 |
|---|---|---|
| 出一套题 | 右侧 AI 助手 | 说"考我 5 道 attention 的题"，完成后题目自动出现 |
| 答题并批改 | 测验 Tab | 在页面作答后点"提交批改"。选择题本地判分，简答由 AI 自动批改，即时出分 |
| 归档旧题 | 测验 Tab | 点"归档" |

### 2.8.4 复习操作指南

**是什么 / 有什么用**：用间隔重复（SRS，基于 SM-2 算法）安排每张卡片的下次复习时间，把精力花在快忘的内容上，对抗遗忘。

| 想做什么 | 在哪做 | 怎么做 |
|---|---|---|
| 新建复习卡片 | 复习 Tab | 点"新建卡片"，填正 / 反面内容 |
| 复习到期卡片 | 复习 Tab | 翻面看答案后，按"重来 / 困难 / 良好 / 容易"四档给自己打分，系统据此排下次复习时间 |
| 暂时不复习某卡 | 卡片操作 | 点"暂停"，需要时再"恢复" |
| 不再复习某卡 | 卡片操作 | 点"归档" |
