...
本节盘点接下来值得做的方向，按三个维度评估：**实用性 / 可展示性（demo 是否吸睛）/ 跟进 AI 潮流**。

## 2.1. 一个判断
**跳出业务看，工程成熟度比再加业务更值钱**

评估 LLM 应用 / Agent / RAG 能力时，"做过一个问答机器人"几乎人人都能说，拉不开差距。真正稀缺的是这句话：**能度量一个 LLM 系统好不好，并能系统性地把它改好、守住、压成本。**
...
复用现有 RAG / Agent / 存储，铺面积、风险低，但差异化一般。
