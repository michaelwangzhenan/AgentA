...
| 编号 | 优先级 | 瓶颈 | 现象（实测） | 修改方案 | 风险与副作用 | 结论 |
|---|---|---|---|---|---|---|
| 1 | P0 | agent 工具循环串行 | 一次问答里每轮 = 1 次 LLM + 工具执行，逐轮串行累加。web 类问答实测 40~75 秒（de4ad2af 7 轮 ~43s；676b 5 轮 ~75s） | ReAct 每轮依赖上一轮结果，本质无法跨轮并行，只能压轮数。两条：① system prompt 已含「失败 query 不原样重发 / 拆子查询」，**待补「鼓励一轮内并发多查」**以减少 LLM 往返轮次；② 轮次上限 `MAX_TOOL_ROUNDS` / `MAX_TOTAL_ROUNDS` / `MAX_HARD_CAP_ROUNDS` 已可配，UI「系统配置→LLM」可改，按场景收紧 | 延迟↔质量权衡，非纯增益：轮数压太狠会漏查导致答案不全；上限调太低会截断复杂任务（plan 自适应放大也救不回）。prompt 引导依赖模型遵循度，弱模型收益有限 | ① **改代码**（补一句 prompt，与 2 互补：减 LLM 往返轮次）；② 配置已可配，无需改 |
| 2 | P0 | 同一轮多个工具调用仍串行 | 一轮要抓多个 URL 时逐个 `fetch_url` 串行（每个几秒）；`tool_call_engine.py` 用 `for tool_call in message.tool_calls` 顺序执行 | 把同一轮 LLM 返回的多个 tool_calls 丢线程池并行执行（工具多为网络 IO，GIL 不挡），执行完再按原顺序写 chat_history / 发 `tool_call_end`；plan 类 tool（make_plan / update_step / abort_plan）排除在并行外、仍串行 | **真增益**，但仅当一轮真有多个独立工具调用时（普通单查无收益）。需处理：① `citation_builder.register` / `chat_history.append` 要线程安全（加锁或回收后串行写）；② 并发抓取可能触发目标站限流；③ 多个 `search_knowledge` 并行会同时吃满 CPU（精排），需配合并发上限 | **改代码**（`tool_call_engine.py` 并行）—— 唯一纯加速项，**优先做** |
| 3 | P1 | 开 thinking 时单轮变慢 | 单轮推理思考 28~32 秒才出内容（676b 第 6 轮 15:37:50→15:38:22） | `THINKING_ENABLED` 默认关、聊天页 Composer 可按需开；`THINKING_BUDGET` 可调，简单问答不开 / 开小档 | 延迟↔质量权衡，非增益项：关 thinking 牺牲复杂推理质量（多步推理 / 规划 / 数学类变差）；budget 调太小思考被截断反而答不准。只是「按场景省时间」 | 配置已可配（对话页），无需改代码 |
| 4 | P1 | 空内容重试多花一轮 | 模型把答案塞进 reasoning、正文留空时，去 tools 重试一轮（多一次完整 LLM 调用）；`agent.py` 已实现：首次遇空 → 强逼一轮正文，仍空才报错 | 维持现有「去 tools 重试一轮」逻辑（鲁棒性优先），且 `force_no_tools` 只重试一次，避免无限重试放大延迟 | 每次触发该轮延迟近翻倍；若某模型频繁空正文体感明显变慢。根因在模型，工程侧只能限次兜底、无法消除。增益是「答得出来」而非「更快」 | 不改（已实现兜底，根因在模型） |
| 5 | P2 | 开发态热重载 + MCP 重连 | 改一次文件 uvicorn 反复重启，每次重连 MCP server ~6 秒（16:08~16:10 多次 reload） | 部署 / 压测用 `--no-reload` 彻底消除；开发态把 reload watch 收窄到 `src/`（已是），避免 `.env` / 日志写入触发重启 | 仅开发态影响，生产无此问题。`--no-reload` 后改代码需手动重启（自用单机可接受）；watch 收窄后 src 外改动需手动重启 | 不改代码（部署用 `--no-reload`，生产无此问题） |
