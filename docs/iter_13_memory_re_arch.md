...
每次触发只调**一次** LLM（不比旧实现多调），输出对记忆列表的操作而非新条目：
...
无状态判定，每轮直接读本 session 累计 user 消息数：

```
触发 = (msg_count > 0) 且 (msg_count % EVERY_N == 0) 且 (len(user_input) >= MIN_INPUT_LEN)
```
...
提取合并在 daemon 后台线程 fire-and-forget，不阻塞本轮回答收尾；`contextvar` 取不到子线程，故主线程先取 `uid` 再显式下传给所有 DB 调用。提取失败静默吞掉。
