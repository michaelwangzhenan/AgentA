...
目标：把 AgentA 限定为个人学习助手与个人资料展示平台。开启后，只处理两类问题：（1）与学习、知识获取、资料整理、编程练习、考试复习、技能训练相关的问题；（2）关于站主个人能力与经历的问答（简历、项目经验、技能特长、职业方向等，供 HR、猎头、用人经理了解站主）。闲聊、娱乐、时政评论、医疗实操、违法咨询等与上述两类无关的请求直接拒答。
...
- 实现范式：固定 LLM 二分类。开启后，每条用户问题在进主对话前额外调用一次 `kimi-k2.5`，判断是否在允许范围内（学习相关 + 个人资料问答），返回 JSON：`{"in_scope": true}` 或 `{"in_scope": false}`。不采用纯关键词规则表，也不做规则与模型混合。
- 无法明确判断是否属于允许话题时，按拒答处理。

验收：开关关闭时无关问题可正常进入 Agent；开关开启时“今天天气怎么样”被拒、“解释这段 Python 报错”可通过、“你有哪些项目经验”可通过；三种聊天路径行为一致；管理后台可查到 `learning_scope` 记录；管理员可在「设置 → 系统配置」切换开关且立即生效。
...
- 固定 system prompt 定义允许话题边界：学习相关（知识获取、资料整理、编程练习、考试复习、技能训练、作业辅导、错题讲解等）与个人资料问答（简历、工作经历、项目经验、技能特长、教育背景、职业方向等）属范围内；闲聊、娱乐、时政评论、医疗实操、违法咨询、旅游攻略、情感陪伴等属范围外。
...
- `section` 设为「内容审核」；`brief` 为「限定问答范围」；`detail` 说明开启后仅允许学习相关话题与个人资料问答，其余拒答，且每条用户问题额外调用一次 `kimi-k2.5` 分类。
