...
# 2. harness 改名为 critic

## 2.1. 起因

agenta 的 "harness" 功能其实是个**很窄的 Critic**（一次性 LLM-judge + flag/删除，无自我纠正回路）。业界 "agent harness" 通常指 LLM 外面整套编排脚手架（大概念），用它命名一个狭义 critic 名实不符。改名为 `critic` 更准确。

`tools/agent_eval/harness/` 那层是**对 critic 的评测**，跟 harness 概念也没关系，一并改。

## 2.2. 重命名映射

| 旧 | 新 |
|---|---|
| `HarnessManager` / `HarnessVerdict` | `CriticManager` / `CriticVerdict` |
| `harness_manager.py` / `get_harness_manager()` | `critic_manager.py` / `get_critic_manager()` |
| `HARNESS_QUIZ_ENABLED` / `HARNESS_RAG_ENABLED` / `HARNESS_GRADING_THRESHOLD` / `HARNESS_LLM_TIMEOUT_SEC` | `CRITIC_QUIZ_ENABLED` / `CRITIC_RAG_ENABLED` / `CRITIC_GRADING_THRESHOLD` / `CRITIC_LLM_TIMEOUT_SEC` |
| DB 列 `quiz_questions.harness_flagged` | `critic_flagged` |
| `tools/agent_eval/harness/` + `eval_harness.py` | `tools/agent_eval/critic/` + `eval_critic.py` |
| 前端 `harness_flagged` 字段 / 文案 | `critic_flagged` |
| 当前文档（design/code/backlog）"harness" 措辞 | "critic"（`iter_*.md` 历史不动） |

类名取 `CriticManager`（与 `*Manager` helper 命名一致）。

## 2.3. 改动要点 / 注意

- **DB 列**：`quiz.db` 里的一个列，用 `ALTER TABLE quiz_questions RENAME COLUMN harness_flagged TO critic_flagged` 原地改名（不丢数据、不建新库）；写一次性迁移脚本对现有 `quiz.db` 跑，跑完即删。`quiz_store.py` 的 schema 校验 + 读写同步改。
- **.env**：单用户本地，直接把 `.env` + `.env.example` 的 `HARNESS_*` 改 `CRITIC_*`。
- **范围**：~25 文件（src / tests / frontend / tools / docs）+ 1 列迁移 + 目录改名。流程：迁移列 → 改代码 → 改文档 → 跑测试。
- **与 backlog §4.6 的重叠**：§4.6 要把 `quiz_critic.txt` / `rag_critic.txt`（生产 prompt）从 `tools/` 挪到 `src/` 修反向依赖；本次只改目录名不挪位置，§4.6 搬迁仍单独做。

## 2.4. 状态

已完成（含 DB 列迁移 + 目录/文件改名 + 全量代码/文档同步 + 测试通过）。
