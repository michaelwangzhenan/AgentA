...
def _dump_report(
    ...
    reports_dir = eval_reports_dir("quiz")
    ...
    # 配对 summary JSON（复用 plan 的 recall+score schema；struct_score=质量均分）
    rate = passed / total if total else 0.0
...
def main() -> None:
    ...
    # 被测模型（生成 quiz plan）= 当前 ACTIVE_MODEL；评委模型 = EVAL_JUDGE_MODEL（UI 可覆盖，回落被测）
