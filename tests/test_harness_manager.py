"""测试 [`src/agent/core/harness_manager.py`](../src/agent/core/harness_manager.py) Phase 2.5 G1 + G2。

覆盖：
    - HarnessVerdict dataclass 字段 + frozen / failure 默认值
    - _load_prompt：文件存在路径 + FileNotFoundError → RuntimeError 含路径
    - HarnessManager.__init__：默认 threshold/timeout 取 config + critic prompt 缓存
    ...
    - 单例 get_harness_manager + reset_for_test
...
from src.agent.core.harness_manager import (
    HarnessManager,
    HarnessVerdict,
    _load_prompt,
    get_harness_manager,
...
@pytest.fixture
def manager(critic_prompts: tuple[Path, Path]) -> HarnessManager:
...
class TestHarnessVerdict:
    ...
    def test_default_failure_false(self) -> None:
        v = HarnessVerdict(passed=True, score=4.0, reason="ok", raw="{}")
        assert v.failure is False

    def test_failure_explicit_true(self) -> None:
        v = HarnessVerdict(passed=True, score=None, reason="超时", raw="", failure=True)
        assert v.failure is True

    def test_frozen(self) -> None:
        v = HarnessVerdict(passed=True, score=4.0, reason="x", raw="")
...
class TestManagerInit:
    ...
    def test_falls_back_to_config_defaults(
        ...
        monkeypatch.setattr(_cfg, "HARNESS_GRADING_THRESHOLD", 4.2)
        monkeypatch.setattr(_cfg, "HARNESS_LLM_TIMEOUT_SEC", 30.0)
        m = HarnessManager(
            quiz_critic_path=quiz_path, rag_critic_path=rag_path,
        )
    ...
    def test_loads_critic_prompts_from_files(
        ...
        quiz_path, rag_path = critic_prompts
        m = HarnessManager(quiz_critic_path=quiz_path, rag_critic_path=rag_path)
...
class TestReviewGrading:
    ...
    def test_pass_when_critic_score_ge_threshold(self, manager: HarnessManager) -> None:
        with patch(
            "src.agent.core.harness_manager.judge_with_llm",
    ...
    def test_fail_when_critic_score_below_threshold(self, manager: HarnessManager) -> None:
        with patch(
            "src.agent.core.harness_manager.judge_with_llm",
    ...
    def test_chat_exception_returns_failure_true(self, manager: HarnessManager) -> None:
        with patch(
            "src.agent.core.harness_manager.judge_with_llm",
...
class TestFilterChunks:
    ...
    def test_empty_hits_returns_empty(self, manager: HarnessManager) -> None:
        assert manager.filter_chunks(query="x", hits=[]) == []

    def test_empty_query_returns_original(self, manager: HarnessManager) -> None:
        hits = [_FakeHit("doc A"), _FakeHit("doc B")]
        assert manager.filter_chunks(query="   ", hits=hits) == hits

    def test_filter_keeps_relevant_drops_irrelevant(self, manager: HarnessManager) -> None:
        hits = [_FakeHit("relevant content"), _FakeHit("noise"), _FakeHit("relevant 2")]
        raw = '{"verdicts": [{"i": 1, "score": 5}, {"i": 2, "score": 0}, {"i": 3, "score": 5}]}'
        with patch(
            "src.agent.core.harness_manager.chat",
    ...
    def test_filter_all_kept_when_all_relevant(self, manager: HarnessManager) -> None:
    ...
    def test_filter_returns_empty_when_all_irrelevant(self, manager: HarnessManager) -> None:
        hits = [_FakeHit("a"), _FakeHit("b")]
    ...
    def test_chat_exception_falls_back_to_original(self, manager: HarnessManager) -> None:
        hits = [_FakeHit("a"), _FakeHit("b")]
        with patch(
            "src.agent.core.harness_manager.chat",
    ...
    def test_parse_failure_falls_back_to_original(self, manager: HarnessManager) -> None:
        hits = [_FakeHit("a"), _FakeHit("b")]
        with patch(
            "src.agent.core.harness_manager.chat",
    ...
    def test_verdicts_length_mismatch_falls_back(self, manager: HarnessManager) -> None:
    ...
    def test_timeout_falls_back_to_original(
        ...
        with patch("src.agent.core.harness_manager.chat", side_effect=_slow_chat):
            kept = m.filter_chunks(query="q", hits=hits)
        assert kept == hits

    def test_truncates_long_chunks(self, manager: HarnessManager) -> None:
        ...
        with patch("src.agent.core.harness_manager.chat", side_effect=_capture_chat):
...
class TestParseRagVerdicts:
    ...
    def test_parse_clean_json(self) -> None:
        ...
        scores = HarnessManager._parse_rag_verdicts(raw, k=2)
        assert scores == [5.0, 0.0]

    def test_parse_with_markdown_block(self) -> None:
        raw = '```json\n{"verdicts":[{"i":1,"score":5}]}\n```'
        scores = HarnessManager._parse_rag_verdicts(raw, k=1)
        assert scores == [5.0]
    ...
    def test_parse_length_mismatch_returns_none(self) -> None:
        raw = '{"verdicts":[{"i":1,"score":5}]}'
        assert HarnessManager._parse_rag_verdicts(raw, k=2) is None

    def test_parse_no_json_returns_none(self) -> None:
        assert HarnessManager._parse_rag_verdicts("plain text", k=1) is None
...
class TestSingleton:
    ...
    def test_get_returns_same_instance(
        ...
        reset_for_test()
        try:
            m1 = get_harness_manager()
            m2 = get_harness_manager()
            assert m1 is m2
        finally:
            reset_for_test()
