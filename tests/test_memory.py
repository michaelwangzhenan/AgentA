...
class TestSetPromptName:
    ...
    def test_set_prompt_name_updates_value(self, store: MemoryStore) -> None:
    ...
    def test_set_prompt_name_can_overwrite(self, store: MemoryStore) -> None:
        store.append("s1", {"role": "user", "content": "hello"})
        store.set_prompt_name("s1", "5g-expert")
    ...
    def test_set_prompt_name_nonexistent_session_is_ignored(self, store: MemoryStore) -> None:
        # session 不存在时不报错，list_sessions 结果不变
        store.set_prompt_name("no-such-session", "5g-expert")
        assert store.list_sessions() == []

    def test_list_sessions_includes_prompt_name_field(self, store: MemoryStore) -> None:
        store.append("s1", {"role": "user", "content": "hello"})
        sessions = store.list_sessions()
        assert "prompt_name" in sessions[0]

    def test_list_sessions_prompt_name_default_is_empty(self, store: MemoryStore) -> None:
        store.append("s1", {"role": "user", "content": "hello"})
        sessions = store.list_sessions()
