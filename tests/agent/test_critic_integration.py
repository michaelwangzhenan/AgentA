"""测试 Phase 2.5 Critic 在 grade_quiz / search_knowledge 主流程上的集成。

覆盖：
    - `_run_quiz_critic` Q1 集成：MCQ 跳过 / short_answer 复审 / 多题混合 flag
    - CRITIC_QUIZ_ENABLED 开关：关时完全跳过 / 开时调 manager
    - CriticManager 初始化失败 → grade_quiz 不阻塞（软返回空 warning 段）
    - critic verdict 持久化到 quiz_questions.critic_flagged 列
    - `_tool_search_knowledge` R1 集成：CRITIC_RAG_ENABLED 开关 / manager 异常软放行
...
from src.agent.core.critic_manager import CriticVerdict
...
def _reset_critic_singleton() -> Iterator[None]:
    hm.reset_for_test()
    yield
    hm.reset_for_test()
...
class TestGradeQuizCriticIntegration:
    ...
    def test_disabled_skips_critic_entirely(
        ...
        """CRITIC_QUIZ_ENABLED=False：grade_quiz 完全跳过 critic，不调 manager。"""
    ...
    def test_critic_skips_mcq_only_short_answer(
        ...
        import src.config as _cfg
        monkeypatch.setattr(_cfg, "CRITIC_QUIZ_ENABLED", True)
        qid, qids = _seed_quiz(store)

        mock_manager = MagicMock()
        mock_manager.review_grading.return_value = CriticVerdict(
            passed=True, score=4.5, reason="批改合理", raw="{}",
        )
        with patch("src.agent.tools._grade_one_short_answer", return_value=(1.0, "对")), \
             patch("src.agent.core.critic_manager.get_critic_manager",
    ...
    def test_critic_flags_short_answer_when_below_threshold(
        ...
        """critic 给 simple short_answer 评低分 → mark critic_flagged + 输出 ⚠️ 块。"""
        import src.config as _cfg
        monkeypatch.setattr(_cfg, "CRITIC_QUIZ_ENABLED", True)
        qid, qids = _seed_quiz(store)
        ...
        assert questions[0]["critic_flagged"] is False  # MCQ 不跑 critic
        assert questions[1]["critic_flagged"] is False
        assert questions[2]["critic_flagged"] is True
    ...
    def test_manager_init_failure_does_not_break_grade(
        ...
        """get_critic_manager() 抛异常（如 critic prompt 文件缺失）→ grade_quiz 软返回空段。"""
        import src.config as _cfg
        monkeypatch.setattr(_cfg, "CRITIC_QUIZ_ENABLED", True)
        qid, qids = _seed_quiz(store)
        with patch("src.agent.tools._grade_one_short_answer", return_value=(1.0, "ok")), \
...
class TestSearchKnowledgeFilterIntegration:
    ...
    def test_disabled_skips_filter(self, monkeypatch: pytest.MonkeyPatch) -> None:
        import src.config as _cfg
        monkeypatch.setattr(_cfg, "CRITIC_RAG_ENABLED", False)
        ...
        with patch("src.agent.tools.search", return_value=hits), \
             ...
             patch("src.agent.core.critic_manager.get_critic_manager") as mgr_call, \
    ...
    def test_enabled_filters_via_manager(
        ...
        import src.config as _cfg
        monkeypatch.setattr(_cfg, "CRITIC_RAG_ENABLED", True)
    ...
    def test_manager_exception_falls_back_to_unfiltered(
        ...
        """get_critic_manager 抛异常 → 保留原始 hits，仍走 format_search_results。"""
        import src.config as _cfg
        monkeypatch.setattr(_cfg, "CRITIC_RAG_ENABLED", True)
