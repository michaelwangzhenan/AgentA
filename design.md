...
将异构本地文档（PDF / DOCX / PPTX / XLSX / MD / HTML / TXT）转换为可被双路检索的索引数据。
...
```mermaid
flowchart LR
    G["golden.json<br/>黄金集"] --> R["对每条 query<br/>跑 search + rerank"]
    R --> M["逐条计算<br/>source / keyword 命中<br/>first_hit_rank"]
    M --> A["汇总<br/>hit@k · MRR"]
    A --> O["落盘 JSON<br/>指标 + metadata + cases"]
```
...
| 指标 | 衡量什么 | 适用场景 |
|---|---|---|
| `hit_source@k` | top-K 内是否命中"期望源文件" | 答案有明确文档归属 |
| `hit_keyword@k` | top-K 内是否包含"期望关键词" | 信息可能跨文件，靠 anchor token 锚定 |
| `hit_either@k` | 上述任一命中（更宽松）| 综合通过率 |
...
- **Results-first JSON 序列化**：字段顺序刻意按"核心指标 → 实验开关 → metadata → 逐条详情"排，打开 JSON 第一屏即看到结果，调优时无需滚动。
- **Metadata 全量留档**：序列化所有"影响结果的配置因子"（git commit + dirty 标志、LLM provider、active embeddings、KB chunk 实测数、reranker / dense 阈值 / BM25 / query 改写 / 切分参数），保证每次实验可追溯、可 diff、可复现。
- **KB chunk 数实测而非读配置**：直接查 ChromaDB `collection.count()`，因为 ingest 历史会让"配置里写的"和"真实落盘的"分叉——避免"配置看起来一样但 KB 已变"造成误判。
- **Ablation 通过 CLI 开关**：`--no-rewriter` / `--no-rerank` 命令行级关闭某组件，无需改 `.env`；JSON 同时记录"该组件最终是否真生效"（已处理静默降级路径），与命令行参数解耦避免误读。
- **失败容忍 · 不污染统计**：单条 query 的 rerank 异常仅 warning 跳过、继续下一条；golden 集缺失时回退到 `golden.example.json` 而不是 hard fail——评估永远应该出报告，哪怕只是"全 0"。
