...
表结构（冗余存 front + back：quiz_question 被 delete 后 SRS 卡仍可独立复习）：
    srs_cards(
        ...
        -- SM-2 调度状态 ---------------------------------------------------------
        ease_factor     REAL    NOT NULL DEFAULT 2.5,      -- 难度因子，下限 1.3
...
跨 session 持久化场景：用户重启 agent → 新 session 问"今天有什么要复习"
...
# SM-2 算法常量（ease 下限 1.3、interval 下限 1）
...
class SRSStore:
    ...
    def __init__(self, db_path: str = SRS_DB_PATH) -> None:
        ...
        # 锁需在建表 / PRAGMA 等任何 _conn 访问前就位（建表也走锁）
    ...
    def update_review_state(
        ...
        *,
        ...
        # 边界保护（ease ≥ 1.3, interval ≥ 1）—— 即便 caller 没夹紧也兜底
