...
class SecurityEventRow(BaseModel):
    event_type: str                       # scrub | tool | ssrf | input_filter | learning_scope | semantic_review
    detail: str
    user_id: int
    created_at: int
...
class SecurityRuntimeSummary(BaseModel):
    range: str
    total: int
    by_type: dict[str, int]               # {scrub, tool, ssrf, input_filter, learning_scope, semantic_review} → 计数
