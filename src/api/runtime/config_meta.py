...
REGISTRY: list[ConfigItem] = [
    ...
    ConfigItem(
        key="SKILL_INJECT_MAX_CHARS",
        group="llm",
        type=ItemType.INT,
        brief="Skill 注入字符上限",
        detail="load_skill 注入会话的正文最大字符数，超出截断。",
        min=1000,
        max=100000,
    ),
    ConfigItem(
    ...
    ConfigItem(
        ...
        type=ItemType.BOOL,
        brief="限定问答范围",
        detail="开启后仅允许学习相关话题与个人资料问答（简历、项目经历等）；其余话题拒答。每条用户问题额外调用一次 kimi-k2.5 分类。",
        section="内容审核",
        danger=True,
    ),
    ConfigItem(
    ...
    ConfigItem(
        key="SEMANTIC_CACHE_MAX_PER_USER",
        group="semantic_cache",
        type=ItemType.INT,
        brief="每用户条数上限",
        detail="单用户语义缓存最多保留条数；超出删最旧。0 表示不限制。",
        min=0,
        max=10000,
    ),
    ConfigItem(
        key="SEMANTIC_CACHE_MAX_GLOBAL",
        group="semantic_cache",
        type=ItemType.INT,
        brief="全局条数上限",
        detail="全站语义缓存最多保留条数；超出删最旧。0 表示不限制。",
        min=0,
        max=100000,
    ),
    ConfigItem(
        key="SEMANTIC_CACHE_MAX_ANSWER_CHARS",
        group="semantic_cache",
        type=ItemType.INT,
        brief="单答案字符上限",
        detail="写入缓存前截断答案正文，防止超大回复占满磁盘与内存。",
        min=1000,
        max=200000,
    ),
    ConfigItem(
