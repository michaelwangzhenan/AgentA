---
name: study-planner
description: 当用户需要规划学习计划、复习计划、考试备考路线、考试冲刺，或希望跟踪 / 更新 / 切换长期学习目标时激活；用 `create_study_plan` 把计划落到 SQLite，跨 session 持久化；用 `update_study_progress` / `query_study_status` 维护进度。新建计划前先用 `make_plan` 把"查领域 → 列阶段 → 列任务 → 落库"拆成 4 步执行。
---
...
- 含"复习计划 / 学习计划 / 学习路线 / 备考 / 考试冲刺 / N 天 / 两周 / 速成"等关键词
- 给定一个学习目标 + 一段时间（"8 周系统学习机器学习"、"两周搞定 Transformer 基础"）
